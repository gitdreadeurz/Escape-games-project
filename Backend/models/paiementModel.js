...
export async function getPayments() {
    const select = "SELECT montant, mode_paiement, promo, p.statut, p.reservation_id, p.estSupprime FROM paiement p INNER JOIN Reservation r ON p.reservation_id = r.reservation_id;";
...
export async function getPaymentById(paiement_id) {
    const select = "SELECT montant, mode_paiement, promo, Paiement.statut, Paiement.reservation_id, Paiement.estSupprime FROM Paiement INNER JOIN Reservation ON Paiement.reservation_id = Reservation.reservation_id WHERE Paiement.paiement_id = ?;";
