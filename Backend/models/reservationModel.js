...
export async function listReservationModel() {
    const list = " SELECT reservation_id, date_reservation, creneau, statut, r.estSupprime, nom, prenom, telephone, mail, r.user_id, titre, localisation FROM reservation r JOIN utilisateur u ON r.user_id = u.user_id JOIN escape_game eg ON r.game_id = eg.game_id;";
    const [result] = await connection.query(list);
    return result;
}
export async function reservationByIdModel(reservation_id) {
    const reservationById = " SELECT reservation_id, date_reservation, creneau, statut, r.estSupprime, nom, prenom, telephone, mail, r.user_id, titre, localisation FROM reservation r JOIN utilisateur u ON r.user_id = u.user_id JOIN escape_game eg ON r.game_id = eg.game_id WHERE r.reservation_id = ?;";
