...
export async function addUser(nom, prenom, role, telephone, mot_de_passe, date_inscription, date_anniv) {
    const add = "INSERT INTO Utilisateur (nom, prenom, role, telephone, mot_de_passe, date_inscription, date_anniv) VALUES (?, ?, ?, ?, ?, ?, ?)";
...
export async function updateUser(user_id, nom, prenom, role, telephone, mot_de_passe, date_inscription, date_anniv) {
    const update = "UPDATE Utilisateur SET nom = ?, prenom = ?, role = ?, telephone = ?, mot_de_passe = ?, date_inscription = ?, date_anniv = ? WHERE user_id = ?";
    const result = await connection.query(update, [nom, prenom, role, telephone, mot_de_passe, date_inscription, date_anniv, user_id]);
    return result;
}
